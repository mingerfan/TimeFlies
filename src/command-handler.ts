...
import { ensureCommandCatalog } from "./command-catalog";
...
export async function handleCommandInput(
  ...
  try {
    const catalog = await ensureCommandCatalog();
    const parsed = parseCommandInput(args.input, catalog);
    result = await executeParsedCommand({
      catalog,
      parsed,
