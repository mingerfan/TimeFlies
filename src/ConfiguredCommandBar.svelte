<script lang="ts">
  ...
  import { ensureCommandCatalog, findCommandDefinitionByToken, listPrimaryCommandNames } from "./command-catalog";
  import type { CommandFeedbackTone } from "./command-executor";
  ...
  let catalog = $state<CommandCatalog | null>(null);
  ...
  const placeholder = $derived.by(() => {
    if (!catalog) {
      return "输入命令或直接输入任务标题";
    }
    const commands = listPrimaryCommandNames(catalog).map((name) => `/${name}`).join(" ");
    return `输入命令（${commands}）或直接输入任务标题`;
  });

  onMount(() => {
    history = loadHistory();
    void ensureCommandCatalog()
      .then((loaded) => {
        catalog = loaded;
      })
      .catch(() => {
  ...
  $effect(() => {
    if (busy || suppressSuggestions || !catalog) {
      suggestions = [];
      return;
    }
    const { items, signature } = buildSuggestions(value, caretIndex, catalog);
    suggestions = items;
  ...
  function buildSuggestions(
    raw: string,
    caret: number,
    currentCatalog: CommandCatalog
  ): { items: Suggestion[]; signature: string } {
    ...
    const commandContext = findCommandContext(raw, caret, currentCatalog);
    if (!commandContext) {
      return { items: [], signature: "" };
    }

    if (commandContext.mode === "command") {
      const items = buildCommandSuggestions(commandContext, currentCatalog);
  ...
  function findCommandContext(
    ...
    const command = findCommandDefinitionByToken(currentCatalog, namePart);
    if (command?.argument?.completion !== "parent_target") return null;
  ...
  function buildCommandSuggestions(
    ...
    const items: Suggestion[] = currentCatalog.commands
      .filter((command) => {
  ...
  function commandInsertText(command: CommandDefinition): string {
    if (command.insert_trailing_space || command.argument?.required) {
      return `${command.name} `;
    }
    return command.name;
  }
